...
double doc_e_step(document* doc, double* gamma, double** phi, lda_model* model, lda_suffstats* ss) /* perform inference on a document and update sufficient statistics */
{
    double likelihood;
    int n, k;
   ...
   likelihood = lda_inference(doc, model, gamma, phi);
    ...
    double gamma_sum = 0; for (k = 0; k < model->num_topics; k++) {
        gamma_sum += gamma[k];
...
void write_word_assignment(FILE* f, document* doc, double** phi, lda_model* model) /* writes the word assignments line for a document to a file * */
{
    int n;
...
void save_gamma(char* filename, double** gamma, int num_docs, int num_topics) /* saves the gamma parameters of the current dataset */
{
    FILE* fileptr;
    int d, k;
...
void infer(char* model_root, char* save, corpus* corpus)
{
	...
	doc = &(corpus->docs[d]);
	phi = malloc(sizeof(double*) * doc->length);
	for (n = 0; n < doc->length; n++)
